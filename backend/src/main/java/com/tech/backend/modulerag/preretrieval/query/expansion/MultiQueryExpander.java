...
package com.tech.backend.modulerag.preretrieval.query.expansion;
...
public class MultiQueryExpander implements QueryExpander {
	...
	public List<Query> expand(@Nullable Query query) {
		...
		log.debug("Generating {} queries for query: {}", this.numberOfQueries, query.text());
		...
		log.debug("MultiQueryExpander#expand() Response from chat client: {}", resp);

		if (Objects.isNull(resp)) {

			log.warn("No response from chat client for query: {}. is return.", query.text());
		...
		if (CollectionUtils.isEmpty(queryVariants) || this.numberOfQueries != queryVariants.size()) {
			...
			log.warn("Query expansion result dose not contain the requested {} variants for query: {}. is return.",
		...
		log.debug("Rewrite queries: {}", queries);
