...
import com.tech.backend.mcp.McpServerUtils;
...
import static com.tech.backend.mcp.McpServerUtils.getMcpLibsAbsPath;
...
public class McpService {
	...
	public ToolCallResp chat(String prompt) {
		...
		log.debug("ToolCalls: {}", toolCalls);
		String responseByLLm = response.getResult().getOutput().getText();
		log.debug("Response by LLM: {}", responseByLLm);
		...
		if (!toolCalls.isEmpty()) {
			...
			log.debug("Start ToolCallResp: {}", tcr);
			...
			try {
				...
				log.error("Error ToolCallResp: {}, msg: {}", tcr, e.getMessage());
...
//				llmCallResponse = toolResponseMessage.getResponses().get(0).responseData();
			...
			tcr.setToolCostTime((long) (tcr.getToolEndTime().getNano() - tcr.getToolStartTime().getNano()));
			log.debug("End ToolCallResp: {}", tcr);
		}
		else {
			log.debug("ToolCalls is empty, no tool execution needed.");
	...
	public ToolCallResp run(String id, Map<String, String> envs, String prompt) throws IOException {
		...
		if (runMcpServer.isEmpty()) {
			log.error("McpServer not found, id: {}", id);
