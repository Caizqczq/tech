...
import com.tech.backend.utils.FilesUtils;
		...
		try {
			response.doFinally(
				...
				try {
					...
					throw new AIException("Error writing to output stream " + e.getMessage());
				}
			});
			...
			throw new AppException("Operation interrupted. " + e.getMessage());
		}
