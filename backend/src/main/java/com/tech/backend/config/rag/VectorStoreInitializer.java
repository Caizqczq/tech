...
package com.tech.backend.config.rag;
...
public class VectorStoreInitializer {
	...
	public void init(VectorStore vectorStore) throws Exception {
		...
		if (markdownDocumentReaderList.isEmpty()) {
			log.warn("No markdown documents found in the directory.");
			return;
		}

		log.debug("Start to load markdown documents into vector store......");
		...
		log.debug("Load markdown documents into vector store successfully. Load {} documents.", size);
	}
	...
	private List<MarkdownDocumentReader> loadMarkdownDocuments() throws IOException, URISyntaxException {
		...
		if (Files.exists(currentDirPath) && Files.isDirectory(currentDirPath)) {
			log.debug("Found markdown directory in current running directory: {}", currentDirPath);
			...
			try (Stream<Path> paths = Files.walk(currentDirPath)) {
				...
				if (!markdownFiles.isEmpty()) {
					log.debug("Loading {} markdown files from current directory", markdownFiles.size());
					...
					log.debug("No markdown files found in current directory, falling back to resources");
				}
			}
		} else {
			log.debug("Markdown directory not found in current directory, falling back to resources");
		}
		
		// 如果当前运行目录没有找到，则从resources目录加载
		Path markdownDir = Paths.get(getClass().getClassLoader().getResource("rag/markdown").toURI());
		log.debug("Loading markdown files from resources directory: {}", markdownDir);
